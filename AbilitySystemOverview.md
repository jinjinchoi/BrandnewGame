...
- [03-4  연속 공격](#03-4-연속-공격-ability)
  - [03-4-1 지연 대처](#03-4-1-지연-대처)
...
### 03-4 연속 공격 Ability
근접 공격 어빌리티 로직은 다음과 같습니다. 공격시 애님 몽타주가 재생되고 Anim notify state를 통해 무기의 콜리전을 설정합니다. 무기에 오버랩 된 액터가 적이라고 판명되면 적에게 대미지를 입힙니다.<br>  
플레이어는 연속 공격을 할 수 있으며 인덱스와 몽타주로 이루어진 Map을 통해 콤보를 이어갑니다. 공격시 인덱스를 통해 몽타주를 가져오고 어빌리티가 종료되면 인덱스를 올린뒤 타이머를 설정해 일정 시간안에 다시 어빌리티가 발동되면 올라간 인덱스에 맞는 애니메이션이 재생되고 타이머가 다 지나면 인덱스가 초기화 됩니다.

#### 03-4-1 지연 대처
콤보 공격의 문제점은 클라이언트와 서버에 지연(Lag)이 발생하면 콤보 인덱스가 다를 수 있다는 점이었습니다. 서버와 클라이언트와의 애니메이션 종료 시점에 차이가 발생하고 이는 곧 인덱스의 차이로 이루어졌습니다. 이를 대처하기 위하여 로컬에서 예측으로 애니메이션을 실행하고 서버에서 보정하는 방식을 도입했습니다.

![콤보 어택 이미지](GameImg/ComboAttack.png)

클라이언트는 먼저 애니메이션을 재생합니다. 서버에서는 클라이언트로 현재 서버의 콤보 공격 인덱스를 보냅니다.

![서버 보정 이미지](GameImg/ComboAttack2.png)

그 후 클라이언트는 자신의 인덱스와 서버에서 보낸 인덱스와 비교하여 동일한지 확인한 후 인덱스가 다르면 다시 애니메이션을 재생 시킵니다.

![시간 보정 이미지](GameImg/ComboAttack3.png)
![콤보 애니메이션 재생 이미지](GameImg/ComboAttack4.png)

클라이언트는 서버와 애니메이션을 최대한 동기화하기 위해 지연된 시간만큼 Start 시간을 조정하여 애니메이션을 재생합니다.<br>  
이를 통해 100~200ms 지연과 3%의 패킷 손실률이 발생하는 상황에서도 약간의 끊김은 발생하지만 콤보 공격은 정상적으로 작동하였습니다.
