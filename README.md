...
로컬 클라이언트가 캐릭터가 복제되면 RPC를 사용해 서버의 Game State에 캐릭터 정보를 저장하고 이를 복제하였는데 Simulated Proxy가 클라이언트에게 존재한다는 것을 인식하지 못한 채 한 실수였고 그로 인해 네트워크 환경이 불안정하면 UI에 다른 플레이어의 정보가 안나오는 버그가 생겼습니다.
