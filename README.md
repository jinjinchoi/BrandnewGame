...
데이터 테이블에서 Row 네임은 `EnemyName_Lv1`과 같은 방식으로 설정되어 Row 별로 Attribute와 에너미를 구분합니다.

데이터 테이블을 사용한 이유는 디자이너가 에너미의 Attribute를 Row 네임을 통해 레벨별로 설정하기 쉽게 하기 위함이었는데 현재는 에너미의 종류 및 레벨 상한이 낮아 이 방식이 유효하지만 프로젝트가 커지면 커브 테이블과 같은 다른 방식을 사용하는 것이 더 적합하다고 판단하고 있습니다.
...
```
캐릭터 클래스에서는 `OverlappedItems` 배열에 아이템 클래스를 저장합니다. 이때 클라이언트의 Simulated Proxy는 아이템 획득 로직과 관련이 없어 제외하였습니다.

클라이언트가 아이템 획득 키를 누르면 클라이언트의 `OverlappedItems`에 아이템이 있는지 먼저 확인합니다. 아이템이 존재하면 RPC를 보내고 서버에서 다시 서버의 `OverlappedItems` 배열을 확인하고 아이템을 획득하는 작업을 합니다.

서버와 클라이언트가 서로 자신의 배열을 사용하게 하여 클라이언트가 임의로 아이템을 획득하는 상황을 방지했습니다.
...
호스트가 게임을 Load를 할 경우 호스트의 서브시스템에는 어느 슬롯을 사용하여 로드를 했는지를 저장합니다.
...
한 가지 번거로웠던 작업은 추적 중인 퀘스트 액터의 위치를 표시하는 작업이었는데 추적 중인 퀘스트를 로드하는 시점이 액터가 서브시스템에 등록되는 시간보다 빠르면 액터를 제대로 찾아올 수 없다는 점이었습니다.
...
그 후 액터를 추가하는 함수에서 추적을 기다리고 있던 액터의 데이터를 받으면 델리게이트를 브로드캐스트 하고 퀘스트 컴포넌트는 이를 수신하여 다시 액터를 찾는 작업을 하도록 하였습니다.
...
이 중 대부분의 문제점은 프록시에 대해 제대로 인지하지 못한 상태에서 발생하였는데 대표적인 문제로 현재는 다른 플레이어의 Player State에 접근하려고 할 때 생겼습니다.

캐릭터 클래스가 생성된 후 Player State가 복제되면 특정 클래스에 Player State를 저장하는 작업을 하였습니다. 그런데 이때 착각을 하여 로컬 클라이언트가 RPC를 사용해 서버에서만 설정하고 저장된 Player State를 복제하여 사용하려고 하였습니다.

하지만 사실 Player State는 이미 복제되어 로컬 클라이언트에게 보여주기 때문에 RPC를 사용해서 복제하는 것이 아닌 Simulated Proxy가 Player State를 저장하면 되는 작업이었습니다.

프록시에 대해 제대로 알지 못해 한 실수로 현재는 프록시들에 대해 더욱 자세히 알게 되었고 덕분에 MD 파일을 작성하면서도 잘못된 RPC 사용이나 과도한 RPC 사용을 수정할 수 있었습니다.
